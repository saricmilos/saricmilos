...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Create notification overlay elements for disabled buttons
    const createNotificationOverlay = (message) => {
    ...
    // Add notification overlays to turbine and economic buttons
    const turbineOverlay = createNotificationOverlay('Please select a location for your wind power plant first');
    const economicOverlay = createNotificationOverlay('Please select a location and a turbine model first');
    
    turbineBtn.appendChild(turbineOverlay);
    economicBtn.appendChild(economicOverlay);
    
    // Add CSS for notification overlays
    if (!document.querySelector('style#overlay-style')) {
        ...
        style.innerHTML = `
            ...
            .nav-button.disabled:hover .notification-overlay {
                opacity: 1;
                visibility: visible;
            }
            .nav-button.disabled-with-hover {
                opacity: 0.6;
                cursor: not-allowed;
            }
            .nav-button.disabled-with-hover:hover .notification-overlay {
                opacity: 1;
                visibility: visible;
            }
        `;
    ...
    turbineBtn.classList.add('disabled');
    economicBtn.classList.add('disabled');
    
    // Function to enable the turbine button when a location is selected
    window.enableTurbineButton = function() {
        turbineBtn.classList.remove('disabled');
        turbineBtn.removeChild(turbineOverlay);
    };
    
    // Function to enable the economic button when a turbine is selected
    window.enableEconomicButton = function() {
        economicBtn.classList.remove('disabled');
        economicBtn.removeChild(economicOverlay);
    };
    ...
    // Only allow clicking the turbine button if it's enabled
    ...
    // Only allow clicking the economic button if it's enabled
...
// Add this function to windlocation.js
function getWindData() {
    ...
    if (!selectedLat || !selectedLng) {
        // Show notification if coordinates aren't selected
        showNotification('Please select a location on the map first', 'warning');
        loadingIndicator.style.display = 'none';
        return;
    }
    
    // In a real application, you would make an API call to the Global Wind Atlas here
    // For demonstration purposes, we'll use the simulateWindData function
    
    // Simulate API call with a slight delay
    setTimeout(() => {
        // Get simulated wind data
        const windData = simulateWindData(selectedLat, selectedLng);
        
        // Display the wind data
        displayWindData(windData);
        
        // Enable the turbine button
        if (window.enableTurbineButton) {
            window.enableTurbineButton();
        }
    }, 1000); // 1 second delay to simulate API call
}

// Modify the displayWindData function in windlocation.js (around line 650-670)
function displayWindData(data) {
    // Store data globally
    globalWindData = data;

    window.globalWindData = data;
    ...
    // Enable the turbine button now that we have wind data
    if (window.enableTurbineButton) {
        window.enableTurbineButton();
    }
...
window.addEventListener('beforeunload', function() {
    // Store wind data in localStorage before page unload
    if (globalWindData) {
        localStorage.setItem('windData', JSON.stringify(globalWindData));
    }
});